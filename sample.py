...
class Login_window:
    def __init__(self, master,*args,**kwargs):
        ...
        #background image
        self.bgImage=ImageTk.PhotoImage(file="C:\\Users\\shyam\\Desktop\\PMS\\Images\\bg4.jpeg")#insert bg4 here
        self.img=Label(master, image=self.bgImage)
        self.img.place(x=0,y=0,relwidth=1, relheight=1)

        #get all images
        self.user_icon=PhotoImage(file="C:\\Users\\shyam\\Desktop\\PMS\\Images\\user1.png")#insert user1 here
        self.password_icon=PhotoImage(file="C:\\Users\\shyam\\Desktop\\PMS\\Images\\password1.png")#insert password1 here
        self.login_icon=PhotoImage(file="C:\\Users\\shyam\\Desktop\\PMS\\Images\\login5.png")#insert login5 here
        ...
        self.btn_login= Button(Login_frame, text="Login", font=('arail 13 bold'), width=10, height=1, bg="firebrick3",
                                    fg='white',command=self.login_sys)
        self.btn_login.grid(row=3,column=1,pady=15)
        #self.master.bind(<"Return">,self.login_sys)
        self.btn_clr = Button(Login_frame, text="Reset", font=('arail 13 bold'), width=10, height=1, bg="firebrick3",
    ...
    def login_sys(self, *args, **kwargs):
        ...
        if(user==str("ABC")) and (pas==str(1234)):
            ...
            tkinter.messagebox.showinfo("Error", "Invalid Username/Password",parent=self.master)
...
class Main_menu:
    def __init__(self, master):
        ...
        #background image
        self.bgImage=PhotoImage(file="C:\\Users\\shyam\\Desktop\\PMS\\Images\\bg.png")#insert bg here
        self.img=Label(master,image=self.bgImage)
        self.img.place(x=0,y=0,relwidth=1, relheight=1)
        ...
        """
        self.heading = Label(master, text="PHARMACY MANAGEMENT", font=('arial 40 bold'), fg='steelblue',bg='gray90')
        self.heading.place(x=300, y=0)
        self.heading = Label(master, text="SYSTEM", font=('arial 40 bold'), fg='steelblue',
                             bg='gray90')
        self.heading.place(x=550, y=65)"""

        #self.heading = Label(master, text="MAIN MENU:", font=('arial 20 bold'), fg='steelblue',
         #                    bg='gray90')
        #self.heading.place(x=280, y=200)

        self.btn_prc = Button(master, text="Purchase", font=('arial 13 bold'), width=18, height=2,
                                bg="steelblue", fg='white',command=self.new_window_purchase)
        self.btn_prc.place(x=270, y=260)

        self.btn_sls = Button(master, text="Sales", font=('arial 13 bold'), width=18, height=2,
                                bg="steelblue", fg='white',command=self.new_window_sales)
        self.btn_sls.place(x=270, y=330)

        self.btn_dlr = Button(master, text="Dealers", font=('arial 13 bold'), width=18, height=2,
                              bg="steelblue", fg='white', command=self.new_window_dealer)
        self.btn_dlr.place(x=270, y=400)

        self.btn_invn = Button(master, text="Inventory", font=('arial 13 bold'), width=18, height=2,
                                bg="steelblue", fg='white',command=self.new_window_inventory)
        self.btn_invn.place(x=270, y=470)

        self.btn_expinvn = Button(master, text="Expired inventory", font=('arial 13 bold'), width=18, height=2,
                                bg="steelblue", fg='white',command=self.new_window_expiredinventory)
...
class dealer:
    def __init__(self, master):
        ...
        self.btn_D_srcname = Button(master, text="Search", font=('arail 13 bold'), width=10, height=1, bg="steelblue",
                                    fg='white', command=self.search_name)
        self.btn_D_srcname.place(x=1220, y=60)

        # button to clear
        self.btn_clear = Button(master, text="Clear all fields", font=('arial 13 bold'), width=18, height=2,
                                bg="steelblue", fg='white', command=self.clear_all)
        self.btn_clear.place(x=360, y=300)

        # button to add to dealer list
        self.btn_D_addtolist = Button(master, text="Add to list", font=('arail 13 bold'), width=18, height=2,
                                      bg="steelblue", fg='white', command=self.add_dealer)
        self.btn_D_addtolist.place(x=562, y=300)

        # button to show all dealers
        self.btn_D_list = Button(master, text="View list of dealers", font=('arail 13 bold'), width=18, height=2,
                                 bg="steelblue", fg='white', command=self.dealer_list)
    ...
    def search_name(self, *args, **kwargs):
        ...
        for x in rows:
            ...
            self.tBox.insert(END, (
                    "   " + fx1 + " " * (6 - len(fx1)) + "| " + fx2 + " " * (26 - len(fx2)) + "| " + fx3 + " " * (
                    14 - len(fx3)) + "| " + fx4))
            self.tBox.insert(END, "\n")
...
class purchase:
    def __init__(self, master):
        ...
        # button to add
        self.btn_add = Button(master, text="Add to inventory", font=('arial 13 bold'), width=18, height=2,
        ...
        self.btn_p_history = Button(master, text="View purchase history", font=('arail 13 bold'), width=18, height=2,
                                    bg="steelblue", fg='white', command=self.purc_history)
        self.btn_p_history.place(x=532, y=700)

        # button for searcing dealer ID
        self.btn_D_ID = Button(master, text="Search", font=('arail 13 bold'), width=10, height=1, bg="steelblue",
                               fg='white', command=self.search_dealer)
...
class sales:
    def __init__(self, master):
        ...
        self.header = Label(self.left, text="SALES", font=('arial 40 bold'), bg='white',
                            fg='steelblue')
        self.header.place(x=250, y=0)

        self.date = Label(self.right, text=str(d.strftime("%A")) + "," + str(d.strftime("%B")) + " " + str(
            d.strftime("%d")) + " " + str(d.strftime("%Y")), font=('arial 12 bold'), bg='lightblue')
        self.date.place(x=200, y=0)

        self.time = Label(self.right, text=str(d.strftime("%X")) + " " + str(d.strftime("%p")), font=('arial 12 bold'),
                          bg='lightblue')
        ...
        self.search = Button(self.left, text="Search", width=25, height=2, bg='orange', activebackground='purple',
                             command=self.data)
        ...
        if self.get_n == "":
            ...
            for i in y:
                if i[0] != None and i[1] != None and i[2] != None:
                    ...
                    self.add = Button(self.left, text="Add to Cart", width=25, height=2, bg='orange',
                                      activebackground='purple', command=self.add_cart)
                    ...
                    self.change = Button(self.left, text="Calculate Change", width=25, height=2, bg='orange',
                                         activebackground='purple', command=self.change_fun)
    ...
    def add_cart(self, *args):
        ...
        if self.quan > int(self.stock):
            ...
            for self.i in prod_list:
                ...
                self.tempp = Label(self.right, text=str(prod_price[self.count]) + '0', font=('arial 18 bold'),
                                   bg='lightblue')
    ...
    def change_fun(self, *args):
        ...
        if self.bal < 0:
            ...
            self.n = Button(self.left, text="Store Name", width=25, height=2, bg='orange', activebackground='purple',
                            command=self.names)
            self.n.place(x=295, y=250)

            self.t_bal = Label(self.left, text="Balance: ₹ " + str(self.bal) + '0', font=('arial 30 bold'), fg='green',
                               bg='white')
    ...
    def names(self, *args):
        ...
        self.bill = Button(self.left, text="Generate Bill", width=90, height=2, bg='red', activebackground='blue',
    ...
    def generate_bill(self, *args):
        ...
        directory = "C:/Users/shyam/Desktop/PMS/Invoice/" + str(dat) + "/"
        ...
        file = "C:/Users/shyam/Desktop/PMS/Invoice/%s/" % str(dat) + "Invoice" + str(
            cd) + ".txt"
        f = open(file, 'w')
        f.write(final)

        for i in range(len(prod_list)):
            f.write("\n\t" + str(i + 1) + "\t  " + str(prod_list[i] + "       ")[:7] + "\t " + str(
        ...
        # Opening the bill
        os.system("C:/Users/shyam/Desktop/PMS/Invoice/%s/Invoice%s.txt" % (
        str(dat), str(cd)))
...
class inventory:
    def __init__(self, master,*args,**kwargs):
        ...
        # button for search
        self.btn_add = Button(master, text="Search", font=('arial 13 bold'), width=18, height=2, bg="steelblue", fg="white",
        ...
        self.btn_view.place(x=500, y=690)
        self.tBox = scrolledtext.ScrolledText(master, width=70, height=40)
    ...
    def search_row(self, *args, **kwargs):
        ...
        for x in rows:
            ...
            fx5 = str(x[4])
            fx6 = str(x[5])
            self.tBox.insert(END, ("   " + fx1 + " " * (27 - len(fx1)) + "| " + fx2 + " " * (9 - len(fx2)) + " | " + fx3 + " " * (
                    9 - len(fx3)) + "| " + fx4 + " " * (4 - len(fx4)) + "| " + fx5 + " " * (
                                3 - len(fx5)) + "| " + fx6 + " " * (3 - len(fx6))))
...
class expired_inventory:
    def __init__(self, master,*args,**kwargs):
        self.master = master
        self.master.title("EXPIRED INVENTORY")
        self.master.geometry("1366x768+0+0")
        self.master.config(bg="powder blue")
        self.frame = Frame(self.master)
        self.frame.pack()
        ...
        self.btn_view.place(x=400, y=690)

        # button to view inventory
        self.btn_view = Button(master, text="VIEW THE INVENTORY", font=('arial 13 bold'), width=20, height=2, bg="lightgreen", fg="white",
                               command=self.inventory_list)
        self.btn_view.place(x=180, y=690)
    ...
    def search_row(self, *args, **kwargs):
        ...
        for x in rows:
            ...
            fx5 = str(x[4])
            fx6 = str(x[5])
            self.tBox.insert(END, ( "   " + fx1 + " " * (14 - len(fx1)) + "| " + fx2 + " " * (10 - len(fx2)) + " | " + fx3 + " " * (
